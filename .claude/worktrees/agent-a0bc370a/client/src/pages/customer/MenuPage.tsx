...
export default function MenuPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      api.getSessionCart(storeId, activeSessionId).then(({ items: serverItems }) => {
  ...
  useEffect(() => {
    ...
    const poll = () => {
      api.getSessionCart(storeId, activeSessionId).then(({ items: serverItems, cartVersion, lastCartSubmitAt }) => {
        ...
        // Update cartVersion in store for submit dedup
        store.setCartVersion(cartVersion)
        // Detect remote cart submission: lastCartSubmitAt changed + server empty + local non-empty
