 ...
 * Returns the items + paymentMode so the route can decide what to do (create order or pay-first).
 */
export function submitSessionCart(
  storeId: string, sessionId: string, expectedVersion: number, customerName?: string,
): { items: CartItem[]; paymentMode: 'pay-first' | 'pay-later'; tableId: string } | { error: string; status?: number } {
  const session = sessionStore.getById(sessionId)
  if (!session) return { error: 'Session not found', status: 404 }
  if (session.storeId !== storeId) return { error: 'Session not found', status: 404 }
  ...
  const allItems = session.pendingCart ?? []
  ...
  // Atomically clear cart and bump version (synchronous — no await gap)
  ...
  logger.info(
    { sessionId, storeId, itemCount: allItems.length, paymentMode },
    'session cart submitted',
  )
