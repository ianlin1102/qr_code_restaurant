...
type Form = { username: string; password: string; role: string; clockPin: string }
const EMPTY: Form = { username: '', password: '', role: 'staff', clockPin: '' }
  ...
  const handleCreate = async () => {
    ...
    try {
      await api.createStaff(storeId, {
        ...
        ...(form.clockPin ? { clockPin: form.clockPin } : {}),
      })
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md w-[calc(100vw-2rem)]">
          ...
          <div className="space-y-3">
            ...
            <Label text={t.clock.pinLabel}>
              <Input value={form.clockPin} maxLength={4} inputMode="numeric"
                onChange={e => {
                  const v = e.target.value.replace(/\D/g, '').slice(0, 4)
                  setForm(f => ({ ...f, clockPin: v }))
                }}
                placeholder={t.clock.pinPlaceholder} />
            </Label>
