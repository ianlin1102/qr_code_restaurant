...
import { staffStore } from './staff.service.js'
import { timeEntryStore } from '../repositories/stores.js'
...
export function verifyPin(
  storeId: string,
  pin: string
): VerifyResult | ServiceError {
  const staff = staffStore.getByField('storeId', storeId)
  const user = staff.find(u => u.clockPin === pin)
  if (!user) {
...
export function clockOut(
  ...
  if (!result.clockedIn || !result.currentEntry) {
    return { error: 'Not clocked in', status: 400 }
  }
...
export function getEntries(
  ...
  if (filters?.startDate) {
    entries = entries.filter(e => e.clockIn >= filters.startDate!)
  }
  if (filters?.endDate) {
    entries = entries.filter(e => e.clockIn <= filters.endDate!)
  }
