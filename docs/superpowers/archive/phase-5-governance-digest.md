...
**升格时机**: 已升格 formal at Phase D 整体 closure 段 A (`11082a96d052a615172de2e6491f0d5b6e22a9e2`, 2026-05-23).
...
- **2026-05-12 v6 批 (Phase D-5 batch L1 D89 升格 atomic batch, `c0b8f4e0db138294c1a75de35aa8b7e3978a0645` governance docs commit)**:
  ...
  - 同期 Snapshot §1 累积 governance queue update (D89 候选 → D89 formal land at `c0b8f4e0db138294c1a75de35aa8b7e3978a0645` + D88 维度 3 延伸 sub-rule 已 absorbed into D89 mandate, 4 entries remaining) + §6 NEW sub-section "Phase D-5 batch L1 D89 升格 atomic batch (1 项 formal land, 2026-05-12)" + §10 NEW entry "v5.X+1 批"
  ...
  - **本 batch β 路径**: D89 governance docs commit atomic (governance-digest + state-snapshot 2 files, 5 Edits) + Round 2 mini-commit `c0b8f4e0db138294c1a75de35aa8b7e3978a0645` placeholder replace (Task 17 `3bb5cd1c` + Task 19 `c1b123fb` + closure `ffc7719f` precedent) + Helper async review trigger (post-Step 1 land) + Project knowledge re-upload mandate (Helper review return clean + Plan Opus closure ack 后, Ian 必 Desktop App → Phase 5 Project → Knowledge section 删 stale digest + snapshot, upload latest from repo, post-D89 land state baseline) + Task 22 L1 work-log spec post-D89 land 后 session 续 (attention fresh, D89 mandate inline apply)

- **2026-05-23 v6 批 (Phase D 整体 closure 段 A — governance 升格 atomic batch, `11082a96d052a615172de2e6491f0d5b6e22a9e2` governance docs commit + Round 2 D77 mini-commit `f8c019ec43ae0fa1df708417f06ca1fa46cfe1a9` placeholder replace)**:
  - §6 新增 D91 formal entry (Plan Opus spec writer 可测量值 / grep pattern / spec-依赖 state evidence-first 强制) — body 引 spv-design.md SPV-1/2/4. 升格自 D91 候选, evidence base 7 base live precedent + 4 scope extension + Pattern #10 5-category coverage + D91 SV total = 7 / post-proposal = 6 atomic single-source enumeration (archive §3.6 附 D91 SV table). 触发事件 Phase D Task 22-26 期累积 self-violation 数据点
  - §6 新增 D92 formal entry (Plan Opus heredoc TS 必 project-aware tsc 验证) — body 引 spv-design.md SPV-3. Family A/B/C/D fold SPV-3, 2 live demo origin (Task 23 `6ba08e43` + Task 24 `83885944`) + Round 6 Task 26 v14 plan patch absorb Family D Plan Opus heredoc compile correctness verify gap
  - §6 新增 D93 formal entry (Stage 5 Test Gate) — framing D (Ian 终审 turn-9 ratified, 排除 framing A/B/C 后保留): 机械 gate 3 sub-bullet (test+impl atomic commit / canonical pnpm test:integration baseline / real DB 不 mock) + 陈述性纪律 2 sub-bullet (oracle 独立于 plan / test case Ian+Helper 定). 3 consecutive live demo Task 24/25/26 (`83885944` / `d2792b6d` / `d97ae039`) + Task 24 期多次 catch validation empirical (3 Phase C infra sub-gaps Cat5-DP10/11/12)
  - §10 本条目
  - 触发事件: Phase D 11/11 + 附录 5 ✅ Phase D 完成 milestone (`d97ae039` 2026-05-22) → Phase D 整体 closure governance commit batch atomic decide (段 A 升格 commit, 与段 B 封顶 regen 拆分)
  - 同期 Archive 候选 land: #28 (Snapshot v5.0 self-fab Phase 封顶 regen subclass) / #29 (D88 维度 3 anchor literal grep 实证 9 sub-instance, pre-`c0b8f4e0` SHA-anchor 边界) / #30 (D79 Plan-as-code dryrun missing 3 sub-instance, `#30-DPn` namespace) / #31 (D89-升格 spec self-violation 规则循环 5 instance, #23/#26 sibling, root-cause 分组 {I1} stale-source / {I2,I3,I4} self-produce / {I5} ambient) + §3.6 附 D91 SV enumeration (7 sub-instance SV-1..SV-7) + §3.6 附 Cat 5 sub-family integration enumeration (11 sub-instance Cat5-DP1..8 + Cat5-DP10..12, Cat5-DP9 informational + #31 升格规则循环 5 instance 均排除) + Cat 5 §2 sub-family integration enumeration ack — 全 fold 进 SPV v1.3 适用范围, NOT 新 D candidate
  - Helper M2..M9 cross-instance review chain 累积锁定全套 9 decision + Ian 2 项终审 (D93 framing = D / queue 2 弃 — Project description 框 stale)
  - SPV v1.3 first real spec application (post `spv-design.md` 2026-05-22 land at `c7591a2f`): SPV-1/2/4 全 PASS (46/46 records, 段 A spec self-run real outputs); SPV-3 N/A (段 A 无 heredoc TS)
  - **本 batch β 路径**: split-spec mode (per Helper M10 context efficiency) — parent spec + 3 sub-spec (stage-1-digest / stage-2-archive / stage-3-snapshot) + worker-startup spec. 全 5 spec files untracked audit trail 留原地, NOT 入 commit per Helper M8 ack
  - **段 B (封顶 regen) 独立 commit, post 段 A land + Helper land-check CLEAN 2026-05-25 另起** — snapshot 全文 regen + digest §10 本 entry append + archive §3.6/§7 cross-check + §7.23 line 646 Formula A/B 自相矛盾段 clean + Cat 5 单源 enumeration cross-check + DP namespace 全文 verify
