...
### 规则 7.2：Plan 写作时的必要性质疑（evidence-first 升级）

规则 7 要求"事实断言必须 grep 证据"，规则 7.2 要求"任务必要性必须显式 verify"。

**触发场景**：plan 写作时涉及下列判断，不能默认成立，必须显式 ask Ian：
- **数据价值判断**：某数据 / 某备份 / 某归档是否有实质价值
- **业务优先级**：某功能 / 某修复是否真的重要
- **历史决策动机**：某现状为什么这样，是有意设计还是 legacy 遗留
- **用户反馈真实性**：某需求是否来自真实用户还是想当然

**原则**：当 "做这件事" 的必要性依赖 Ian domain knowledge（代码 / grep 不能证明），plan 写作时必须 ask 不能默认。

**反模式**：
- ❌ 凭 plan 常见模式写 "需要备份 EC2 数据" 没问 Ian 是否真需要
- ❌ 假设 "用户期望 X 功能" 没问 Ian 是否真的有用户
- ❌ 把 "spec 惯例" 当 "本项目事实"

**正确做法**：
- ✅ 写 plan 时若遇 "是否需要 X / 某物是否有价值" 类判断，显式标 `[NEEDS IAN CONFIRMATION]` 并在提交 plan 前 ask
- ✅ plan 中明示假设 "基于 Ian 未 verify 的 assumption: ..."

**历史教训**：
- 2026-04-17 Phase D Task 17 itemKey 模型错误（规则 7 应用不足，可 grep）
- 2026-04-17 Phase D Task 16 stores.ts 方向性错误（规则 7 应用不足，可 grep）
- **2026-04-19 Phase A-1 全 skip 事件（规则 7.2 应用不足，需 Ian 判）**

第三次事件触发规则 7.2 升格。本规则对 plan 写作阶段（spec / plan 本身）同样适用，不只是实施阶段。
...
- **D62**：webhook 幂等机制——选定候选 B（`Payment.stripePaymentIntentId UNIQUE` partial index + DB 层冲突幂等）。Phase G 段 5 Task 41 plan §3 正式登记,5 条理由（DB 层强制 0 应用层代码 / Stripe PI ID 天然幂等 / 无需过期清理 / 事务边界简单 / migration 成本可控）。落地依赖 Phase B schema 增量 migration（`@@index → @@unique` partial index `WHERE stripePaymentIntentId IS NOT NULL`）+ webhook handler 入口 `paymentRepo.findByStripeId` fast-path + P2002 catch（详 handoff §"D62 候选 B 落地依赖" + 2026-04-19 追加块）。spec 回填编号 **D62**。
- **Task 40 编号悬空**：Phase G 段 5 Task 39 + Task 40 合并为单 Task 39（B.2 编号处理,scope-check 发现 service 层完全被 Task 38 C6b2 §2.1/§2.2 吃掉,routes 层留 Task 39 处理）。锚点 scope-check work-log `20e69b30`。Phase G 段 5 最终 task 数 = 3（39/41/42,非原交接包 4）。
...
| G | 3c | [phase-g-session-order.md](./phase-g-session-order.md)（Task 32-33, 415 行）+ [phase-g-session-cart-b2.md](./phase-g-session-cart-b2.md)（Task 34, 565 行, 含 D58）+ [phase-g-b2-checkpoint.md](./phase-g-b2-checkpoint.md)（Task 35, 478 行）+ [phase-g-payment-service.md](./phase-g-payment-service.md)（Task 36, 435 行, 含 D59/D60/D61）+ [phase-g-settlement-gateway.md](./phase-g-settlement-gateway.md)（Task 37 part 1, 368 行, 含 **D63**）+ [phase-g-settlement-gateway-part2.md](./phase-g-settlement-gateway-part2.md)（Task 37 part 2, 411 行）+ [phase-g-settlement-actions.md](./phase-g-settlement-actions.md)（Task 38 part 1, 412 行）+ [phase-g-settlement-split-bill.md](./phase-g-settlement-split-bill.md)（Task 38 part 2, 419 行）+ [phase-g-split-bill-routes.md](./phase-g-split-bill-routes.md)（Task 39 合并, 372 行, **Task 40 悬空** B.2 锚点 work-log `20e69b30`）+ [phase-g-webhook.md](./phase-g-webhook.md)（Task 41, 505 行, 含 **D62 决议**）+ [phase-g-session-payment-settlement.md](./phase-g-session-payment-settlement.md)（Task 42, 491 行, 三方原子 commit 协调） | **10**（Task 32-42 含 Task 40 悬空, plan 5/5 段全部完成 ✅, 实施未启动） | Phase E/F |
...
| 39 | **合并 Task 39+40**（B.2 编号处理,Task 40 悬空,锚点 scope-check work-log `20e69b30`）—— `split-bill.routes.ts` B2 适配（6 routes handlers tenantAwareRoute + 调 D61 controller 边界 parseItemKey）+ handoff §2 5 处 .split(':') 最终 verify 回收 → [phase-g-split-bill-routes.md](./phase-g-split-bill-routes.md) |
| 41 | webhook B2 适配 + **D62 决议正式拍板候选 B**（`Payment.stripePaymentIntentId UNIQUE` partial index + DB 层冲突幂等,5 条理由）+ 5 [Task 41 final verify] markers 落实（R-X1 时序图 / afterCommit hook 适配 / tx context tenant / manual capture metadata D59 协调）→ [phase-g-webhook.md](./phase-g-webhook.md) |
| 42 | session-payment + session-settlement B2（13 + 10 JsonStore + itemKey FK signature + C5b2 范围 3 derivePaidState/3 .split(':') 整合 + confirmItemPayment 保留+FK+测试同步）+ **C6b1 + C6b2 + Task 42 三方原子 commit 协调要求** → [phase-g-session-payment-settlement.md](./phase-g-session-payment-settlement.md) |
