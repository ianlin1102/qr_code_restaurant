...
## 7. 治理改进候选 D74(源自本次规则 8 触发 + Ian α 决议)

**追加时间**:2026-04-19,Task 7 β refinement commit(`77baab58`)落地后,本轮对话治理产出。

**候选标题**:Plan 修订前置行数预算 + 实时预警

**触发事件**:Task 7 β 修订 edit 完成后,`git diff --stat` 显示 **+75 行**,超 Ian 指令 "~50 行(β 合理上限)" **50%**。CC 规则 8 暂停汇报,Ian α 决议接受原版。

**机制设计**:
- **时机**:plan 修订指令含明示行数上限时,CC edit **前** 先估算每区块行数
- **预警**:edit 中累计行数达 **50% 预算** 时自检触发,判断继续 edit 还是暂停汇报
- **反馈**:edit 完成后 `git diff --stat` verify,如超预算再次 flag(现有规则 8 回退路径)
- **例外机制**:若内容 0 冗余(如本次 75 行全部真决议产出),Ian 可 α 决议接受超预算;事后**复盘预算设值**(50 行是否偏紧)

**与既有规则关系**:

| 规则 | 性质 | 时机 |
|---|---|---|
| **规则 8** | 反应式 | edit 完成超预算 → 暂停汇报 |
| **规则 8.1** | 主动式(pending commits 外化) | 实时维护清单 |
| **D74** | 主动式(行数预算外化) | edit 前估 + 中预警 |

D74 和规则 8 互补,非替代。规则 8 依然承担最终回退路径;D74 降低触发概率。

**升格目标**:Phase H Task 45 spec reconcile 时,与 D67-D73 候选统一处理(D67-D71 Task 2 修订 + D72/D73 Task 7 β refinement + **D74 治理改进**)。

**CC 工作流自我升级**:下次 plan 修订指令起,CC 在 edit 前输出 "行数预算估算"(基于指令中的上限 + 内容块清单),让 Ian 在 edit 前就有机会调预算或缩 scope。
